...
export const addRapidBlocks: TelegramCommand = {
	name: CALLBACKS.ADD_RAPID_BLOCK,
	description: "Adds a rapid block cycle.",
	async execute(ctx: Context) {
		...
		if (!userId) {
			...
			if (!userSchedule) {
				...
				const sessionBlocks = allBlocks.default.blocks;
				...
				if (userSchedule.schedule && userSchedule.schedule.length > 0) {
					const lastBlock = userSchedule.schedule.reduce((latest, curr) => {
						...
						const latestEnd =
							new Date(latest.startAt).getTime() +
							latest.durationMin * 60 * 1000;
						return currEnd > latestEnd ? curr : latest;
					});
					const lastEnd =
						new Date(lastBlock.startAt).getTime() +
						lastBlock.durationMin * 60 * 1000;
